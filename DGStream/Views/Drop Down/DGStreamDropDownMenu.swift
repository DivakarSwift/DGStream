...
open class DGStreamDropDownMenu: UIView, DGStreamDropDownMenuDelegate {
    internal var opened: Bool = false
    internal var openedIndex: Int = 0
    
    internal var dropDownButtons: [DGStreamDropDownButton]?
    ...
    internal var dropDownViews: [UIView]? {
        ...
        set {
            ...
            _dropDownViews.flatMap({ $0 as? DGStreamDropDownView }).forEach { $0.delegate = self }
    ...
    public init(frame: CGRect, YNDropDownViews: [DGStreamDropDownView], dropDownViewTitles: [String]) {
        super.init(frame: frame)
    ...
    @objc internal func menuClicked(_ sender: DGStreamDropDownButton) {
    ...
    internal func showMenu(yNDropDownButton: DGStreamDropDownButton?, buttonImageView: UIImageView?, dropDownView: UIView?, didComplete: (()-> Void)?) {
        guard
        ...
        (dropDownView as? DGStreamDropDownView)?.dropDownViewOpened()
    ...
    internal func hideMenu(yNDropDownButton: DGStreamDropDownButton?, buttonImageView: UIImageView?, dropDownView: UIView?, didComplete: (()-> Void)?) {
        guard
            let yNDropDownButton = yNDropDownButton,
            let dropDownView = dropDownView else { return }
        
        (dropDownView as? DGStreamDropDownView)?.dropDownViewClosed()
    ...
    internal func initViews() {
        ...
        for i in 0..<numberOfMenu {
            // Setup button
            let button = DGStreamDropDownButton(frame: CGRect(x: eachWidth * CGFloat(i), y: 0.0, width: eachWidth, height: CGFloat(menuHeight)), buttonLabelText: dropDownViewTitles?[i])
            button.tag = i
